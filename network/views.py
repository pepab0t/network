...
def following_page(request):
    return render(request, "network/index.html", {
        'script': 'network/following_page.js',
        'title': 'Following'
    })
...
def follow(request, user_id: int):
    ...
    if (status := request.GET.get('status', None)) is None:
        return JsonResponse({'error': 'query parameter `status: bool` not specified'}, status=400)
    ...
    if status == 'true':
        ...
        return JsonResponse({'message': f'Successfully following user {user}', 'followers': followers}, status=200)
    else:
        f = Follow.objects.filter(follower=request.user, follows=user).first()
        if f is None:
            return JsonResponse({'error': f'Follow ({request.user} -> {user}) not found'}, status=404)
        f.delete()
        followers = user.followers.count() # type: ignore
        return JsonResponse({'message': f'User {request.user} successfully unfollowed {user}', 'followers': followers}, status=200)
...
def user_detail(request, username: str):
    ...
    return JsonResponse({"followers": user.followers.count(), "following": user.following.count(), "username": user.username,
                         "i_am_follower": i_am}, status=200)
...
def like(request, post_id: int):
    ...
    if like is None:
        ...
        return JsonResponse({'message': f'Successfully added like to post {post}', "likes": post.likes.count()}, status=200)
    else:
        like.delete()
        return JsonResponse({'message': f'Successfully unliked post {post}', "likes": post.likes.count()}, status=200)
