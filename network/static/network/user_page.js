...
const fetchPosts = generateFetchPostsFn(`http://localhost:8000/posts/${myGlobal.viewedUser}`);

document.addEventListener('DOMContentLoaded', () => {
    fetchPosts().then(displayPosts);

    fetch(`http://localhost:8000/profile/${myGlobal.viewedUser}/detail`)
    ...
    .then(detail => {
        ...
        if (btnFollow !== null) {
            ...
            btnFollow.onclick = async () => {
                const detail = await fetch(`http://localhost:8000/profile/${myGlobal.viewedUser}/detail`).then(response => response.json());
                const data = await fetch(`http://localhost:8000/follow/${myGlobal.viewedUserId}?status=${!detail.i_am_follower}`).then(response => response.json());
                if (data.error) {
